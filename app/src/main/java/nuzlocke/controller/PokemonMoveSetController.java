...
public class PokemonMoveSetController {
    ...
    @PostMapping
    public ResponseEntity<PokemonMoveSet> createMoveSet(
            @RequestHeader(value = "Idempotency-Key", required = false) String key,
            @RequestBody PokemonMoveSet newMoveSet) throws JsonMappingException, JsonProcessingException {
        log.info("creating new move set for pokemon: " + newMoveSet.getPokemon().getPokemonName());
        return ResponseEntity.status(HttpStatus.CREATED).body(moveSetService.createNewMoveSet(key, newMoveSet));
    }
